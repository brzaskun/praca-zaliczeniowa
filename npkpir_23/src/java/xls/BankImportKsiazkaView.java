...
public class BankImportKsiazkaView implements Serializable {
 ...
 //typ transakcji
    ...
    public void init() { //E.m(this);
        rodzajedokKlienta = new ArrayList<>(Arrays.asList("wb","wbe","wbm","wbme", "rev", "wbm_pom", "wbme_pom"));
...
//    
                    ...
                    if (zwrot.size()==5) {
                        ...
                        if (naglowek==null) {
                         ...
                         for (Iterator<ImportBankWiersz> it = pobranefaktury.iterator(); it.hasNext();) {
                             ...
                             if (wie.getWnma().equals("Ma")||(!wie.getNrtransakji().contains("PRZELEW")&&!wie.getNrtransakji().contains("PRZYCH")&&!wie.getNrtransakji().contains("BLIK"))) {
                                 it.remove();
                             }
                         }
                    }
    ...
    public void usun(ImportBankWiersz r) {
     ...
     public int generowanieDokumentu(ImportBankWiersz r, Waluty walutadokumentu) {
        ...
        try {
            FakturaRozrachunki nowe = new FakturaRozrachunki(r, wpisView.getPodatnikObiekt(), wpisView.getUzer(), rodzajdok, wpisView.getRokWpisuSt(), mc, walutadokumentu.getSymbolwaluty());
            if (r.getKurs()!=0.0) {
