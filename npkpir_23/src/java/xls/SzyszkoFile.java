...
//    }
     ...
     public static Object[] getListafaktur(byte[] plikinterpaper, List<Klienci> k, KlienciDAO klienciDAO, String rodzajdok, String jakipobor, String mc) {
        ...
        List<InterpaperXLS> importyzbrakami = Collections.synchronizedList(new ArrayList<>());
         try {
            InputStream file = new ByteArrayInputStream(plikinterpaper);
            List<String> list = new BufferedReader(new InputStreamReader(file, Charset.forName("UTF-8"))).lines().collect(Collectors.toList());
                ...
                for (CSVRecord row :recordss) {
                    if (i==1) {
                        ...
                        try {
                            ...
                            String mcdok = Data.getMc(Data.zmienkolejnosc(row.get(21)));
                            if (mc.equals(mcdok)) {
...
//FVPR i FVPR 2  Sprzedaz z przekroczeniem limitu gdzie stosowana ma być stawka danego kraju (DE  16% i FR 20%). 
    ...
    private static CSVRecord uzupelnijsprzedaz2(InterpaperXLS interpaperXLS, CSVRecord row, List<Klienci> k, KlienciDAO klienciDAO, Map<String, Klienci> znalezieni) {
        ...
        try {
                String nip = pobierznip(row);
                interpaperXLS.setNrfaktury(row.get(25));
                interpaperXLS.setDatawystawienia(Data.stringToDate(Data.zmienkolejnosc(row.get(22))));
                interpaperXLS.setDatasprzedaży(Data.stringToDate(Data.zmienkolejnosc(row.get(21))));
                interpaperXLS.setDataobvat(Data.stringToDate(Data.zmienkolejnosc(row.get(21))));
                interpaperXLS.setKlientnazwa(row.get(9));
                 String krajsymbol = row.get(14);
                String kraj = PanstwaMap.getWykazPanstwXS().get(krajsymbol);
                interpaperXLS.setKlientpaństwo(kraj);
                interpaperXLS.setKlientkod(row.get(14));
                interpaperXLS.setKlientmiasto(row.get(12));
                interpaperXLS.setKlientulica(row.get(11));
                ...
                interpaperXLS.setWalutaplatnosci(row.get(32));
                interpaperXLS.setSaldofaktury(Z.z(Double.parseDouble(row.get(28))));
//                interpaperXLS.setNettoPLN(Z.z(Double.parseDouble(row.get(26))));
...
//                interpaperXLS.setBruttoPLN(Z.z(Double.parseDouble(row.get(29))));
                interpaperXLS.setNettowaluta(Z.z(Double.parseDouble(row.get(26))));
                interpaperXLS.setVatwaluta(Z.z(Double.parseDouble(row.get(27))));
                interpaperXLS.setBruttowaluta(Z.z(Double.parseDouble(row.get(28))));
            } catch (Exception e) {
            zlafaktura = row;
        }
        return zlafaktura;
