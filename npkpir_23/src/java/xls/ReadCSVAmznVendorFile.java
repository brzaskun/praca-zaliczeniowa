...
public class ReadCSVAmznVendorFile {
...
//    }
     ...
     public static Object[] getListafaktur(byte[] plikinterpaper, List<Klienci> k, KlienciDAO klienciDAO, String rodzajdok, String jakipobor, String mc) {
        ...
        List<InterpaperXLS> importyzbrakami = Collections.synchronizedList(new ArrayList<>());
         try {
            InputStream file = new ByteArrayInputStream(plikinterpaper);
            List<String> list = new BufferedReader(new InputStreamReader(file, Charset.forName("windows-1250"))).lines().collect(Collectors.toList());
...
Iterable<CSVRecord> records = csvFormat.parse(targetReader);
                ...
                for (CSVRecord row :recordss) {
                    if (i==1) {
                        ...
                        try {
                            ...
                            String mcdok = Data.getMc(row.get(2).replace(".", "-"));
                            if (mc.equals(mcdok)) {
...
//FVPR i FVPR 2  Sprzedaz z przekroczeniem limitu gdzie stosowana ma być stawka danego kraju (DE  16% i FR 20%). 
    ...
    private static CSVRecord uzupelnijsprzedaz2(InterpaperXLS interpaperXLS, CSVRecord row, List<Klienci> k, KlienciDAO klienciDAO, Map<String, Klienci> znalezieni) {
        ...
        try {
                ...
                interpaperXLS.setDatawystawienia(Data.stringToDate(row.get(2).replace(".", "-")));
                interpaperXLS.setDatasprzedaży(Data.stringToDate(row.get(2).replace(".", "-")));
                interpaperXLS.setDataobvat(Data.stringToDate(row.get(2).replace(".", "-")));
                interpaperXLS.setKlientnazwa(row.get(4));
                String krajsymbol = row.get(10);
                ...
                interpaperXLS.setKlientmiasto(row.get(9));
                interpaperXLS.setKlientulica(row.get(5));
                ...
                interpaperXLS.setWalutaplatnosci(row.get(11));
                //interpaperXLS.setSaldofaktury(Z.z(Double.parseDouble(row.get(28))));
                interpaperXLS.setNettoPLN(Z.z(Double.parseDouble(row.get(15))));
                interpaperXLS.setNettoPLNvat(Z.z(Double.parseDouble(row.get(15))));
                interpaperXLS.setVatPLN(Z.z(Double.parseDouble(row.get(16))));
                interpaperXLS.setBruttoPLN(Z.z(Double.parseDouble(row.get(17))));
                interpaperXLS.setNettowaluta(Z.z(Double.parseDouble(row.get(12))));
                interpaperXLS.setVatwaluta(Z.z(Double.parseDouble(row.get(13))));
                interpaperXLS.setBruttowaluta(Z.z(Double.parseDouble(row.get(14))));
            } catch (Exception e) {
            zlafaktura = row;
        }
        return zlafaktura;
