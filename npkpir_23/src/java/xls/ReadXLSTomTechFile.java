...
public class ReadXLSTomTechFile {
    ...
    public static List<InterpaperXLS> getListafaktur(byte[] plikinterpaper, String mc) {
     ...
     public static List<InterpaperXLS> getListafakturXLS(byte[] plikinterpaper, List<Klienci> k, KlienciDAO klienciDAO, String rodzajdok, String mc) throws Exception{
        List<InterpaperXLS> listafaktur = Collections.synchronizedList(new ArrayList<>());
        InputStream file = new ByteArrayInputStream(plikinterpaper);
   ...
   private static void uzupelnijzakup(InterpaperXLS interpaperXLS, Row row, List<Klienci> k, KlienciDAO klienciDAO, Map<String, Klienci> znalezieni) {
        if (row.getCell(0).getRowIndex()>0) {
            interpaperXLS.setNrfaktury("brak "+row.getCell(0).getRowIndex());
            ...
            interpaperXLS.setDataobvat(row.getCell(1).getDateCellValue());
            String kontr = row.getCell(2).getStringCellValue();
            interpaperXLS.setKontrahent(kontr);
            interpaperXLS.setNip(row.getCell(2).getStringCellValue().replace("-", "").replaceAll("\\s+","").trim());
            interpaperXLS.setKlient(ustawkontrahenta(interpaperXLS, k, klienciDAO, znalezieni));
            interpaperXLS.setWalutaplatnosci("PLN");
            if (row.getCell(3).getNumericCellValue()!=0.0 || row.getCell(4).getNumericCellValue()!=0.0) {
                interpaperXLS.setKoszt0material1(false);
            } else {
                interpaperXLS.setKoszt0material1(true);
            }
            ...
            interpaperXLS.setBruttowaluta(row.getCell(9).getNumericCellValue());
            interpaperXLS.setNettoPLN(row.getCell(8).getNumericCellValue());
            interpaperXLS.setNettoPLNvat(row.getCell(8).getNumericCellValue());
            interpaperXLS.setVatPLN(row.getCell(10).getNumericCellValue());
            interpaperXLS.setOpis(row.getCell(11).getStringCellValue());
        }
   }
