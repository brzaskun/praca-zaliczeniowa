...
public class PdfSymulacjaWyniku {
    ...
    private static PdfPTable tablica(WpisView wpisView, List<SaldoKonto> listakonta, String pk, int rodzajdruku) throws DocumentException, IOException {
        ...
        for (SaldoKonto rs : listakonta) {
            ...
            table.addCell(ustawfrazeAlign(rs.getObrotyWn() != 0 ? formatujliczby(rs.getObrotyWn()) : "", "right", 7));
            table.addCell(ustawfrazeAlign(rs.getObrotyMa() != 0 ? formatujliczby(rs.getObrotyMa()) : "", "right", 7));
            table.addCell(ustawfrazeAlign(rs.getSaldoWn() != 0 ? formatujliczby(rs.getSaldoWn()) : "", "right", 7));
            table.addCell(ustawfrazeAlign(rs.getSaldoMa() != 0 ? formatujliczby(rs.getSaldoMa()) : "", "right", 7));
   ...
   private static PdfPTable tablica2(List<SymulacjaWynikuView.PozycjeSymulacji> listapozycjisymulacji) throws DocumentException, IOException {
        ...
        for (SymulacjaWynikuView.PozycjeSymulacji rs : listapozycjisymulacji) {
            ...
            table.addCell(ustawfrazeAlign(formatujliczby(rs.getWartosc()), "right", 7));
        }
        return table;
    }
}
