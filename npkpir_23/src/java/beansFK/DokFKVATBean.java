...
//                }
     ...
     public static void pobierzkontaZpoprzedniegoDokumentu(Dokfk poprzedniDokument, Dokfk selected) {
        try {
            if (poprzedniDokument != null) {
                int rozmiar = poprzedniDokument.getListawierszy().size();
                if (poprzedniDokument.getListawierszy().size()>2) {
                    rozmiar = 3;
                }
                        ...
                        if (wierszDokumentuPoprzedniego != null && wierszDokumentuBiezacego != null) {
                            try {
                                ...
                                if (wnDokumenuBiezacego.getKonto()!=null&&!wnDokumenuBiezacego.getKonto().getPelnynumer().startsWith("221")) {//nie ruszamy konta vat zakupowego
                                    if (wnDokumenuBiezacego !=null && wnDokumentuPoprzedniego != null && !wnDokumentuPoprzedniego.getKonto().getZwyklerozrachszczegolne().equals("vat") && !selected.getRodzajedok().isTylkovatnalezny()) {
                            ...
                            try {
                                ...
                                if (maDokumenuBiezacego!=null && maDokumentuPoprzedniego != null && !maDokumentuPoprzedniego.getKonto().getZwyklerozrachszczegolne().equals("vat")) {
    ...
    public static void rozliczVatKoszt(EVatwpisFK wierszvatdoc, WartosciVAT wartosciVAT, Dokfk selected, Map<String, Konto> kontadlaewidencji, WpisView wpisView, Dokfk poprzedniDokument, Konto kontoRozrachunkowe, Cechazapisu nkup) {
        ...
        try {
            ...
            if (wierszpierwszy != null && wierszpierwszy.getStronaWn().getKwota() == 0.0) {
                ...
                if (wartosciVAT.netto == 0.0) {
                    ...
                    if (w.getSymbolwaluty().equals("PLN")) {
                        ...
                        if (ewidencje.contains(selected.getRodzajedok().getRodzajtransakcji())) {
                            ma.setKwota(wartosciVAT.netto);
                            ma.setKwotaPLN(wartosciVAT.netto);
                        } else {
            ...
            if (selected.getRodzajedok().getProcentkup()!=0.0) {
                rozliczobcieciekosztow(selected, nkup, kontadlaewidencji.get("404-2"));
            }
    ...
    private static void rozliczobcieciekosztow(Dokfk selected, Cechazapisu nkup, Konto vatnkup) {
        double procent = Z.z(selected.getRodzajedok().getProcentkup()/100);
        List<Wiersz> wierszenkup = new ArrayList<>();
        int nrkolejnegowiersza = selected.getListawierszy().size()+1;
        for (StronaWiersza p : selected.getStronyWierszy()) {
            if (p.isWn() && p.getKonto().getPelnynumer().startsWith("4")) {
                ...
                Wiersz wiersznkup = ObslugaWiersza.utworzNowyWierszWn(selected, nrkolejnegowiersza++, nowawartoscnkup, 1);
                wiersznkup.getStronaWn().setKwotaPLN(Z.z(nowawartoscnkupPLN));
                wiersznkup.getStronaWn().setKwotaWaluta(Z.z(nowawartoscnkupWaluta));
                if (p.getWiersz().getOpisWiersza().contains("podatek vat nie podl. odl.")) {
                    wiersznkup.getStronaWn().setKonto(vatnkup);
                } else {
    ...
    private static void sprawdzsumyobciecie(List<Wiersz> listawierszy) {
        try {
            ...
            double roznica = Z.z(sumaMa-sumaWn);
    ...
    private static void dolaczwiersz2_3(WartosciVAT wartosciVAT, Waluty w, int lp, int odliczenie0koszt1, Dokfk selected, Map<String, Konto> kontadlaewidencji) {
         Wiersz wiersz2_3;
                ...
                if (odliczenie0koszt1==0   && !selected.getRodzajedok().isTylkovatnalezny()) {
                    ...
                    if (ewidencje.contains(selected.getRodzajedok().getRodzajtransakcji()) && !selected.getRodzajedok().isTylkovatnalezny()) {
                        wiersz2_3.getStronaMa().setKonto(kontadlaewidencji.get("221-1"));
                    }
                }
    ...
    public static void rozliczVatKosztNapraw(EVatwpisFK wierszvatdoc, WartosciVAT wartosciVAT, Dokfk selected, WpisView wpisView, Cechazapisu nkup, Konto kontoRozrachunkowe, Map<String, Konto> kontadlaewidencji) {
        Wiersz wierszpierwszy = selected.getListawierszy().get(0);
        Waluty w = selected.getWalutadokumentu();
    ...
    public static void rozliczVatKosztNaprawWB(Wiersz wierszpierwszy,Dokfk selected) {
      ...
      public static void rozliczVatKosztEdycja(EVatwpisFK wierszvatdoc, WartosciVAT wartosciVAT, Dokfk selected, WpisView wpisView, Cechazapisu nkup, int rowindex, Map<String, Konto> kontadlaewidencji) {
        Wiersz wierszpierwszy = selected.getListawierszy().get(0);
        Waluty w = selected.getWalutadokumentu();
        try {
                StronaWiersza wn = wierszpierwszy.getStronaWn();
