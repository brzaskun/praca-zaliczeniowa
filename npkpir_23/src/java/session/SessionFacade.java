...
//    }
    ...
    public Pitpoz findPitpoz(String rok, String mc, String pod, String udzialowiec, Cechazapisu cecha) {
        Pitpoz tmp = null;
        if (cecha==null) {
            tmp = (Pitpoz) em.createQuery("SELECT p FROM Pitpoz p WHERE p.pkpirR = :pkpirR AND p.pkpirM = :pkpirM AND p.podatnik = :podatnik AND p.udzialowiec = :udzialowiec AND p.cechazapisu IS NULL").setParameter("pkpirR", rok).setParameter("pkpirM", mc).setParameter("podatnik", pod).setParameter("udzialowiec", udzialowiec).getSingleResult();
        } else {
            tmp = (Pitpoz) em.createQuery("SELECT p FROM Pitpoz p WHERE p.pkpirR = :pkpirR AND p.pkpirM = :pkpirM AND p.podatnik = :podatnik AND p.udzialowiec = :udzialowiec AND p.cechazapisu = :cecha").setParameter("pkpirR", rok).setParameter("pkpirM", mc).setParameter("podatnik", pod).setParameter("udzialowiec", udzialowiec).setParameter("cecha", cecha).getSingleResult();
        }
        return tmp;
    }
    ...
    public List<Dok> findDokBKodMcadoMca(Podatnik pod, String rok, String mcod, String mcdo) {
        return Collections.synchronizedList(em.createNamedQuery("Dok.findByBKodMcadoMca").setParameter("podatnik", pod).setParameter("pkpirR", rok).setParameter("mcod", mcod).setParameter("mcdo", mcdo).getResultList());
    }
    ...
    public Dok findDokTPR(String typdokumentu, Podatnik pod, String rok) {
        Dok d = null;
        d = (Dok) em.createNamedQuery("Dok.findByTPR").setParameter("podatnik", pod).setParameter("pkpirR", rok).setParameter("typdokumentu", typdokumentu).setMaxResults(1).getSingleResult();
        return d;
    }
...
//    }
    ...
    public List<Konto> findStronaByPodatnikRokKontoDist(Podatnik podatnik, String rok) {
        return Collections.synchronizedList(em.createNamedQuery("StronaWiersza.findStronaByPodatnikRokKontoDist").setParameter("podatnikObj", podatnik).setParameter("rok", rok).setParameter("rokK", Integer.parseInt(rok)).getResultList());
    }
...
//    }
    ...
    public List<EVatwpisFK> findEVatwpisFKByPodatnikRokMcodMcdo(Podatnik podatnik, String rok, String mcod, String mcdo) {
        return Collections.synchronizedList(em.createNamedQuery("EVatwpisFK.findByPodatnikRokMcodMcdo").setParameter("podatnik", podatnik).setParameter("rok", rok).setParameter("mcod", mcod).setParameter("mcdo", mcdo).setHint(QueryHints.QUERY_RESULTS_CACHE, HintValues.TRUE).getResultList());
    }
...
//    }
    ...
    public Collection<? extends Klienci> findKontrahentFakturyRO(Podatnik podatnikObiekt) {
        return Collections.synchronizedList(em.createNamedQuery("Faktura.findByKonrahentPodatnik").setParameter("podatnik", podatnikObiekt).setHint(QueryHints.QUERY_RESULTS_CACHE, HintValues.TRUE).setHint(QueryHints.READ_ONLY, HintValues.TRUE).getResultList());
    }
