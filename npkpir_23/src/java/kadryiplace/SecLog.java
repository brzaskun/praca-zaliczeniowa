 ...
 */
@Entity
@Table(name = "sec_log", catalog = "kadryiplace", schema = "dbo")
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "SecLog.findAll", query = "SELECT s FROM SecLog s"),
    @NamedQuery(name = "SecLog.findByActSerial", query = "SELECT s FROM SecLog s WHERE s.actSerial = :actSerial"),
    @NamedQuery(name = "SecLog.findByActDatetime", query = "SELECT s FROM SecLog s WHERE s.actDatetime = :actDatetime"),
    @NamedQuery(name = "SecLog.findByActUserid", query = "SELECT s FROM SecLog s WHERE s.actUserid = :actUserid"),
    @NamedQuery(name = "SecLog.findByActTable", query = "SELECT s FROM SecLog s WHERE s.actTable = :actTable"),
    @NamedQuery(name = "SecLog.findByActObject", query = "SELECT s FROM SecLog s WHERE s.actObject = :actObject"),
    @NamedQuery(name = "SecLog.findByActKeyCols", query = "SELECT s FROM SecLog s WHERE s.actKeyCols = :actKeyCols"),
    @NamedQuery(name = "SecLog.findByActKeyValues", query = "SELECT s FROM SecLog s WHERE s.actKeyValues = :actKeyValues"),
    @NamedQuery(name = "SecLog.findByActUpdateCols", query = "SELECT s FROM SecLog s WHERE s.actUpdateCols = :actUpdateCols"),
    @NamedQuery(name = "SecLog.findByActOldValues", query = "SELECT s FROM SecLog s WHERE s.actOldValues = :actOldValues"),
    @NamedQuery(name = "SecLog.findByActNewValues", query = "SELECT s FROM SecLog s WHERE s.actNewValues = :actNewValues")})
public class SecLog implements Serializable {
    ...
    public SecLog(Integer actSerial) {
        this.actSerial = actSerial;
    }

    public SecLog(Integer actSerial, Date actDatetime, String actUserid, String actTable) {
        this.actSerial = actSerial;
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof SecLog)) {
            return false;
        }
        SecLog other = (SecLog) object;
    ...
    public String toString() {
        return "kadryiplace.SecLog[ actSerial=" + actSerial + " ]";
    }
