    ...
    public void pobierzdane() {
        if (selectedpracownik!=null) {
            ...
            for (Iterator<Pasekwynagrodzen> it = paskiwybranego.iterator(); it.hasNext();) {
                ...
                if (p==null||(p!=null&&p.getDefinicjalistaplac()!=null&&p.getDefinicjalistaplac().getRodzajlistyplac()!=null&&p.getRodzajWynagrodzenia()==1006)) {
                    it.remove();
                }
            }
                        ...
                        if (pasek.isPrzekroczenieoddelegowanie()) {
                            ...
                            if (pasek.isZlecenie()==false) {
                                ...
                                obliczpodatekwstepnyDBStandard(pasek, nowapodstawaumowaoprace, stawkipodatkowe, 0.0);
                                 ...
                                 zdrowotnaodliczenie(pasek.getPraczdrowotnedoodliczenia(), pasek);
            ...
            Predicate<Pasekwynagrodzen> isqualified = item->item.getKalendarzmiesiac()==null;
            paskiwybranego.removeIf(isqualified);
            paskiwybranego.add(paseksuma);
        } else {
            paskiwybranego = new ArrayList<>();
        }
    }

    public  void zdrowotnaodliczenie(double praczdrowotnedoodliczenia, Pasekwynagrodzen pasek) {
        if (pasek.getRokI()<2022) {
            pasek.setPrzekroczenienowazdrowotnadoodliczenia(praczdrowotnedoodliczenia);
            double nowypodatek = Z.z0(pasek.getPrzekroczenienowypodatek()-praczdrowotnedoodliczenia)>0.0?Z.z0(pasek.getPrzekroczenienowypodatek()-praczdrowotnedoodliczenia):0.0;
            pasek.setPrzekroczenienowypodatek(nowypodatek);
        }
