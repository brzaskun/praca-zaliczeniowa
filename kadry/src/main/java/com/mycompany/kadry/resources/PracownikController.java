...
public class PracownikController implements Serializable {
    ...
    public void pracownikaktywuj(Pracownik pracownik) {
        wpisView.setPracownik(pracownik);
    }

    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/messagesbundle/Bundle").getString("PracownikCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/messagesbundle/Bundle").getString("PracownikUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/messagesbundle/Bundle").getString("PracownikDeleted"));
        if (!JsfUtil.isValidationFailed()) {
    ...
    public List<Pracownik> getItems() {
        if (items == null) {
            items = getFacade().findAll(Pracownik.class);
        }
        return items;
    }
    ...
    public Pracownik getPracownik(java.lang.Integer id) {
        return (Pracownik) getFacade().findEntity(Pracownik.class, id);
    }

    public List<Pracownik> getItemsAvailableSelectMany() {
        return getFacade().findAll(Pracownik.class);
    }
