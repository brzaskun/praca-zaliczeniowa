...
public class PdfUmowaoDzielo {
    ...
    public static ByteArrayOutputStream drukuj(Umowa umowa, String nazwa) {
        ...
        try {
            if (umowa != null) {
                ...
                dodajtresc(umowa, document, umowa.getWynagrodzeniegodzinowe(), umowa.getWynagrodzeniemiesieczne());
    ...
    //NOWE DRUKOWANIE
    public static ByteArrayOutputStream drukujwszystkie(Umowa umowa, FirmaKadry firmaKadry, List<Angaz> angazlista, double wynagrodzeniegodzinowe, double wynagrodzeniestale) {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        try {
            ...
            for (Angaz an : angazlista)  {
                ...
                umowa.setAngaz(an);
                dodajtresc(umowa, document, wynagrodzeniegodzinowe, wynagrodzeniestale);
                licznik++;
            }
            finalizacjaDokumentuQR(document,nazwa);
            Plik.zapiszBufferdoPlik(nazwa, out);
    ...
    private static void dodajtresc(Umowa umowa, Document document, double wynagrodzeniegodzinowe, double wynagrodzeniestale) {
        try {
...
//            document.add(new Paragraph(new Phrase("pieczątka firmy", fontS)));
            document.add(Chunk.NEWLINE);
            paragraph = new Paragraph(new Phrase("UMOWA O DZIEŁO", font));
            paragraph.setAlignment(Element.ALIGN_CENTER);
            document.add(paragraph);
//            paragraph = new Paragraph(new Phrase(umowa.getCzastrwania().replace("biezacy", ""), fontM));
//            paragraph.setAlignment(Element.ALIGN_CENTER);
//            document.add(Chunk.NEWLINE);
            paragraph = new Paragraph(new Phrase("zawarta w dniu "+umowa.getDatazawarcia()+" pomiędzy Zamawiającym:", fontM));
            document.add(paragraph);
            ...
            document.add(paragraph);
            paragraph = new Paragraph(new Phrase("a Wykonawcą", fontM));
            document.add(paragraph);
            ...
            PdfMain.dodajLinieOpisuSpacing(document, "Zamawiający powierza wykonanie, a Wykonawca zobowiązuje się wykonać dzieło polegające na: "+umowa.getStanowisko(), Element.ALIGN_LEFT, 1, 10);
...
//            } else {
//            }
            String terminpracy = "1. Termin rozpoczęcia dzieła strony ustaliły na dzień "+umowa.getDataod()+"a zakończenie i wydanie Zamawiającemu na dzień "+umowa.getDatado();
            String terminpracy2 = "2. W razie zwłoki w wykonaniu dzieła Zamawiający może odstąpić od umowy bez konieczności wyznaczania dodatkowego terminu";
            PdfMain.dodajLinieOpisuSpacing(document, terminpracy, Element.ALIGN_LEFT, 1, 10);
            PdfMain.dodajLinieOpisuSpacing(document, terminpracy2, Element.ALIGN_LEFT, 1, 10);
            PdfMain.dodajLinieOpisuBezOdstepu(document, "§3", Element.ALIGN_CENTER, 1);
            String par3 = "Odbiór nastąpi w siedzibie Wykonawcy na podstawie protokółu przekazania sporządzonego w obecności przedstawicieli Zamawiającego i Wykonawcy. ";
            ...
            String rekla1 = "1. W przypadku wystąpienia w dziele wad, Zamawiający prześle wykonawcy protokół reklamacyjny, a Wykonawca zobowiązany jest odpowiedzieć w ciągu 7dni.";
            String rekla2 = "2. Brak odpowiedzi w wyznaczonym terminie uważa się za uznanie tej reklamacji, z obowiązkiem załatwienia jej zgodnie z żądaniem Zamawiającego."; 
            PdfMain.dodajLinieOpisuSpacing(document, rekla1, Element.ALIGN_LEFT, 1, 10);
            PdfMain.dodajLinieOpisuSpacing(document, rekla2, Element.ALIGN_LEFT, 1, 10);
            PdfMain.dodajLinieOpisuBezOdstepu(document, "§5", Element.ALIGN_CENTER, 1);
            if (wynagrodzeniegodzinowe>0.0){
                String par4 = "Z tytułu wykonania dzieła Wykonawca otrzyma wynagrodzenie w wysokości: "+umowa.pobierzwynagrodzenieString(wynagrodzeniegodzinowe)+" za godzinę";
                PdfMain.dodajLinieOpisuSpacing(document, par4, Element.ALIGN_JUSTIFIED, 1, 10);
            } else if (wynagrodzeniestale > 0.0) {
                String par4 = "Z tytułu wykonania dzieła Wykonawca otrzyma wynagrodzenie w wysokości: "+umowa.pobierzwynagrodzenieString(wynagrodzeniestale)+" miesięcznie";
                PdfMain.dodajLinieOpisuSpacing(document, par4, Element.ALIGN_JUSTIFIED, 1, 10);
            }

            PdfMain.dodajLinieOpisuBezOdstepu(document, "§6", Element.ALIGN_CENTER, 1);
            String par5 = "Wypłata wynagrodzenia nastąpi po wystawieniu rachunku przez Wykonawcę i stwierdzeniu przez Zamawiającego terminowego i prawidłowego wykonania dzieła będącej przedmiotem niniejszej umowy.";
            ...
            String par6 = "Dane osobowe Wykonawcy podlegają ochronie zgodnie z obowiązującymi przepisami. Wykonawca wyraża zgodę na przetwarzanie danych osobowych w celach ewidencyjnych, podatkowych i ubezpieczeniowych przez Zamawiającego, zgodnie z obowiązującymi przepisami o systemie ubezpieczeń społecznych oraz o podatku dochodowym od osób fizycznych.";
            ...
            String par9 = "Spory mogące wyniknąć z realizacji niniejszej umowy będą rozstrzygane przez sąd właściwy rzeczowo dla siedziby Zamawiającego.";
            PdfMain.dodajLinieOpisuSpacing(document, par9, Element.ALIGN_JUSTIFIED, 1, 10);
            
            PdfMain.dodajLinieOpisuBezOdstepu(document, "§11", Element.ALIGN_CENTER, 1);
            ...
            document.add(p);
            p = new Paragraph();
            p.add(new Phrase("(data i podpis Wykonawcy)", fontS));
            p.setTabSettings(new TabSettings(250));
            p.add(Chunk.TABBING);
            p.add(new Phrase("(podpis Zamawiającego lub osoby reprezentującej)", fontS));
            document.add(p);
        } catch (Exception ex) {
            E.e(ex);
        }
     ...
     private static void dodajtrescAneks(Zmiennawynagrodzenia zm, Angaz angaz, Document document, String datazmiany, boolean netto0brutto1) {
        try {
            ...
            if (umowa.getUmowakodzus().isPraca()) {
                ...
                p.setTabSettings(new TabSettings(300));
                p.add(Chunk.TABBING);
                p.add(new Phrase("(podpis Zamawiającego lub osoby rep. Zlec.)", fontS));
                document.add(p);
            }
        } catch (Exception ex) {
            E.e(ex);
